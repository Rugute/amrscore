...
/**
 * Abstract base class for report builders which build cohort reports - i.e. one row-per-patient
 * dataset
 */
public abstract class AbstractCohortReportBuilder extends AbstractReportBuilder {
	 ...
	 *      AbstractReportBuilder#getParameters(org.openmrs.module.kenyacore.report.ReportDescriptor
	 *      )
	 */
	@Override
	...
	protected void addStandardColumns(CohortReportDescriptor report, PatientDataSetDefinition dsd) {
		...
		if (report.getDisplayIdentifier() != null) {
			...
			DataDefinition identifierDef = new ConvertedPatientDataDefinition("identifier",
			        new PatientIdentifierDataDefinition(idType.getName(), idType), identifierFormatter);
