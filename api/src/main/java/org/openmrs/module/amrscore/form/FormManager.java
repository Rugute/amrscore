...
package org.openmrs.module.amrscore.form;
...
import org.openmrs.module.amrscore.CoreUtils;
import org.openmrs.module.amrscore.calculation.CalculationUtils;
import org.openmrs.module.amrscore.form.FormDescriptor.Gender;
import org.openmrs.module.amrscore.program.ProgramDescriptor;
import org.openmrs.module.amrscore.program.ProgramManager;
...
public class FormManager implements ContentManager {
	...
	private void refreshTagHandlers() {
		for (TagHandler tagHandler : Context.getRegisteredComponents(TagHandler.class)) {
			...
			if (className.endsWith(tagHandlerClassSuffix)) {
				String tagName = StringUtils.uncapitalize(className.substring(0,
				    className.length() - tagHandlerClassSuffix.length()));
	...
	/**
	 * Filters the given collection of enabled forms to those applicable for the given application
	 * and patient
	 * 
	...
	public boolean isPatientEligibleFor(Patient patient, FormDescriptor descriptor) {
		...
		CalculationResult result = Context.getService(PatientCalculationService.class)
		        .evaluate(patient.getId(), calculation);
