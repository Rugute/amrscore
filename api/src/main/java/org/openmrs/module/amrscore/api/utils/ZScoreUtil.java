...
public class ZScoreUtil {
	...
	/**
	 * Reads a json file with weight for length values The file is a json object with length (to 1
	 * decimal point) as the object keys. This is to facilitate O(1) searching
	 * 
	 * @param fileName
	 * @return
	 */
	...
	/**
	 * Calculates weight for lenght z-score based on a client's weight, height, and sex NOTE: This
	 * only works for children under 5 yrs Based on a client's height in cm, the precomputed weights
	 * and standard deviations are obtained. The client's weight is then mapped to classify the
	 * patient. The result is the weight in the matrix which is the immediate <= the client weight
	 * 
	 * @param height
	 * @param weight
	 * @param sex
