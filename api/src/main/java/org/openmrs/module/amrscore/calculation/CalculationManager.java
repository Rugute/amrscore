...
package org.openmrs.module.amrscore.calculation;
...
public class CalculationManager implements ContentManager, CalculationProvider {
	...
	public synchronized void refresh() {
		...
		for (BeanDefinition bd : scanner.findCandidateComponents("org.openmrs.module")) {
			try {
				Class<? extends PatientCalculation> clazz = (Class<? extends PatientCalculation>) Context.loadClass(bd
				        .getBeanClassName());
	...
	public Calculation getCalculation(String calculationName, String configuration) throws InvalidCalculationException {
		...
		if (clazz == null)
			throw new InvalidCalculationException("Not Found: " + calculationName + " (valid values are: "
			        + calculationClasses.keySet() + ")");
